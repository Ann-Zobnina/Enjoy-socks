...
const apiAddedSockRouter = Router();

apiAddedSockRouter.post('/sock/like', async (req, res) => {
...
apiAddedSockRouter.post('/sock/basket', async (req, res) => {
  const { colorId, decorId, imageId } = req.body;
  console.log(req.body, '123123123123', res.locals?.user?.id);
  try {
    // Создаем товар (носок) и сохраняем его
...
apiAddedSockRouter.post('/sock/add', upload.fields([
...
export default apiAddedSockRouter;
